...
const DIFF_IMB_COLORS=["#14532d","#16a34a","#4ade80","#86efac","#dcfce7","#fee2e2","#fca5a5","#f87171","#ef4444","#991b1b"];
...
function renderDiffLegend(absThresholds,mode,metric,palette){
  palette=palette||DIFF_BIN_COLORS;
  const isIMBDiff=metric==="失衡系数";
  ...
  hdr.innerHTML=isIMBDiff
    ?'<span style="flex:1;text-align:center;background:rgba(20,83,45,0.12);color:#15803d;border-radius:6px;padding:2px 0;">失衡收窄（绿）</span><span style="flex:1;text-align:center;background:rgba(220,38,38,0.1);color:#dc2626;border-radius:6px;padding:2px 0;">失衡加剧（红）</span>'
...
function renderDiffComparison(){
  ...
  if(!cells.length){
    ...
    const diffPalette0=metric==="失衡系数"?DIFF_IMB_COLORS:DIFF_BIN_COLORS;
    onMapReady(()=>{updateHeatLayer({type:"FeatureCollection",features:[]},diffPalette0,10);renderDiffLegend(emptyAbs,diffMode,metric,diffPalette0);});
  ...
  showLayer(FILL,true);
  showLayer(LINE,true);
  const diffPalette=isDiffIMB?DIFF_IMB_COLORS:DIFF_BIN_COLORS;
  onMapReady(()=>{updateHeatLayer({type:"FeatureCollection",features},diffPalette,10);renderDiffLegend(absThresholds,diffMode,metric,diffPalette);});
